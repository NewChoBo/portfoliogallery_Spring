...
public class PortfolioService {
	...
	public ApiResult getPortfolioList(Pageable pageable) {
		...
		long total = queryFactory.select(portfolio.count()).from(portfolio).fetchOne();
	...
	public ApiResult deletePortfolio(long portfolioId) {
		long result = queryFactory.delete(portfolio).where(portfolio.portfolioId.eq(portfolioId)).execute();
		if (result == 1)
			return ApiUtils.success().build();
		else
